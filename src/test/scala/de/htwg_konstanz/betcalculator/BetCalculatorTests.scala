...
class BetCalculatorTests extends UnitTestConfiguration {
  ...
  private var betCalculator: BetCalculator = _
  private val allBets: Map[Int, Bet] = Map(
      1 -> Bet(1,1.1), 
      2 -> Bet(4,1.1), 
      3 -> Bet(0, 1.3))

  override def beforeEach() {
    betCalculator = new BetCalculator(allBets, 2)
  }

  test("3 quotes should result in 3 pairs, when invoked with 2 combinations") {
    val actualPairs = betCalculator.createCombinations2
